...
| `src/evaluation/metrics.py` and `figures.py` | Do not exist yet | Week 6–8 |
...
|---|---|---|
| Fig 1 — System Architecture | `docs/architecture.md` (draw.io manually) | Pending |
| Fig 2 — MIMIC-IV Fidelity | `src/data/fidelity.py` → `results/figures/fig2_fidelity_distributions.{png,pdf}` | Pending |
| Fig 3 — FL Convergence Curves | `src/evaluation/figures.py` (not yet created) | Week 8 |
| Fig 4 — Five-Model Bar Chart | `src/evaluation/figures.py` (not yet created) | Week 8 |
| Fig 5 — Privacy-Utility Tradeoff | `src/dp/epsilon_sweep.py` → `results/figures/fig5_dp_privacy_utility.{png,pdf}` | Pending |
...
## Current Status Snapshot (as of Session 3)

All source code scaffolded and importable. Synthetic data generated (CSVs in `data/synthetic/`).
Schema and loader tests written. Architecture doc and literature map notebooks done.

**Immediate next steps:**
1. Run `pytest tests/test_schema.py tests/test_loaders.py -v` to verify all tests pass
2. Add the 2 missing BibTeX entries to `paper/references.bib`
3. Upload paper to Overleaf and verify LaTeX compiles
4. Apply for MIMIC-IV PhysioNet access (takes ~2 weeks)
5. Week 6: run FL simulation (`python -m src.fl.simulation --all`)

The 16-week timeline lives in the git commit history and the weekly plan tracked externally.
