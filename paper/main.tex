...
closely approaching the Centralised Oracle (0.9824) while outperforming global
FedAvg (0.9685) by 1.42 AUC points overall (+2.65pt on IL, +0.36pt on SNF)
(Mann-Whitney $p < 0.001$). Critically, the IL advantage (+2.65pt) is
substantially larger than the SNF advantage (+0.36pt), confirming that
domain-driven clustering is most impactful for the highest non-IID cluster.
...
the synthetic SNF mismatch target (28\%). Differential privacy at
$\varepsilon = 10$ introduces 15.7\% AUC-ROC degradation relative to the
no-DP baseline; tighter budgets ($\varepsilon \leq 5$) degrade utility by
$> 27\%$, motivating larger models or tree-level DP for stricter requirements.
...
Dashed line marks recommended operating point ($\varepsilon = 10$, AUC-ROC 0.8288,
15.7\% degradation). Dotted line marks 95\% of no-DP AUC.
$\varepsilon \leq 5$ causes $> 27\%$ utility degradation with the current model size.}
\label{fig:dp}
\end{figure}
...
We recommend $\varepsilon=10$ as the operating point: AUC 0.8288
(15.7\% degradation from no-DP), offering clinically acceptable utility
...
\multicolumn{5}{l}{\small Mann-Whitney U=400, $p < 0.001$ (CFL vs FedAvg per-round AUC).}
\end{tabular}
\end{table}
...
clients. CFL stabilises above FedAvg across 50 rounds
(per-round gap confirmed by Mann-Whitney U=400, $p < 0.001$).}
...
Mann-Whitney U on per-round AUC distributions confirms statistical significance
(U=400, $p < 0.001$). CFL closely approaches the Centralised Oracle (0.9824)
...
tradeoff. At our recommended operating point $\varepsilon = 10$,
AUC-ROC is 0.8288 — a 15.7\% degradation from the no-DP baseline (0.9829).
Tighter budgets ($\varepsilon \leq 5$) degrade utility by $> 27\%$, suggesting
...
Opacus DP-SGD at $\varepsilon = 10$ achieves AUC-ROC of 0.8288 (15.7\%
degradation from no-DP), while $\varepsilon \leq 5$ degrades utility by
$> 27\%$ with the current model size. This result is consistent with the
