...
def plot_model_comparison():
    ...
    ax.set_ylabel("AUC-ROC (held-out facilities 6 & 9)", fontsize=10, labelpad=6)
...
# Fig 2 is generated by src/data/fidelity.py (the canonical C2 pipeline: KS-test
# vs real MIMIC-IV + within-MIMIC-IV cohort calibration). Run that module
# directly to regenerate it -- do not replot it here with a synthetic-holdout
# proxy, which would silently revert Fig 2 to the pre-MIMIC-IV placeholder.
...
def main():
    ...
    logger.info("Fig 2 (fidelity distributions) is owned by src/data/fidelity.py -- skipping here")
